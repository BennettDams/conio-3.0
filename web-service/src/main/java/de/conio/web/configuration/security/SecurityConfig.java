...
import de.conio.web.connector.consumer.RoleConsumer;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public SecurityConfig(UserDetailsService userDetailsService, PasswordEncoder passwordEncoder,
			RedirectAuthenticationSuccessHandler successHandler, RoleConsumer roleConsumer) {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		reg = reg.antMatchers("/webjars/**").permitAll();
		reg = reg.antMatchers("/css/**").permitAll();
		reg = reg.antMatchers("/media/**").permitAll();
		reg = reg.antMatchers("/lib/**").permitAll();

		// Require authentication for all URLS ("/**")
		reg = reg.antMatchers("/**").hasAnyAuthority(roleConsumer.findAllWithNameOnly().toArray(new String[0]));
		...
		login = login.loginPage(WebApplication.LOGIN_URL).failureUrl(WebApplication.LOGIN_FAILURE_URL)
				.successHandler(successHandler);
		login.and().logout().logoutSuccessUrl(WebApplication.APP_URL);
	}
