...
import de.conio.jpa.behaviour.service.RoleService;
import de.conio.jpa.behaviour.service.UserService;
import de.conio.jpa.behaviour.service.post.PostBookService;
import de.conio.jpa.behaviour.service.post.PostCategoryService;
import de.conio.jpa.behaviour.service.post.PostTutorialService;
...
public class DatabaseSeeder {
	...
	private void seedPostTable() {
		...
		if (rs == null || rs.size() <= 0) {
			PostCategoryEntity pc = postCategoryService.read(1);
	...
	private void seedUserTable() {
		...
		List<PostAttachment> rs = jdbcTemplate.query(sql, (resultSet, rowNum) -> null);
		if (rs == null || rs.size() <= 0) {
			UserEntity u1 = new UserEntity("Bennett", "mail@bennett", userService.encodePassword("admin"));
			UserEntity u2 = new UserEntity("Jonas", "mail@jonas", userService.encodePassword("admin"));
